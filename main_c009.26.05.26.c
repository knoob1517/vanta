    ...
    if (p->current.type == VTOKEN_IDENTIFIER) {
        ...
        if (p->current.type == VTOKEN_ASSIGN) {
            ...
            if (is_variable_defined(ident.value)) {
                ...
                n->let.value = val;
                return n;
            } else if (skip_error) {} else printf("runtime error: undefined variable '%s'\n", ident.value); exit(1);
        }
...
typedef struct { int type; double num; char* s; } Value;
int break_flag = 0;
int continue_flag = 0;
int return_flag = 0;
Value return_value;
    ...
    switch (node->type) {
        ...
        case AST_BUILTIN: {
            ...
            switch(node->builtin.type) {
                case VTOKEN_LEN: v.type = 0; v.num = (arg.type == 1) ? (int)strlen(arg.s) : 0; break;
                ...
                case VVTOKEN_TYPE: v.type = 0; v.num = arg.type; break;
                case VTOKEN_UPPER: 
        ...
        case AST_BINOP: {
            ...
            else if (node->binop.op == VTOKEN_EQ)    {v.type = 2; v.num = fabs(l.num - r.num) < tol;}
            else if (node->binop.op == VTOKEN_NEQ)   {v.type = 2; v.num = fabs(l.num - r.num) >= tol;}
        ...
        case AST_CALL: {
            ...
            if (skip_error) {} else {printf("runtime error: function '%s' not found\n", node->call.name); exit(1);}
        }
    }
    return v;
}
