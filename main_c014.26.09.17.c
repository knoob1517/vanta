...
char* read_string(Lexer* l) {
    ...
    while (l->current != '"' && l->current != '\0') {
        ...
        if (l->current == '\\') {
            ...
            else if (l->current == 'r') buffer[i++] = '\r';
    ...
    if (l->current != '"') {
        error_alert(l, "unterminated string");
    }
...
char* read_multiline_string(Lexer* l) {
    char* buffer = NULL;
    size_t capacity = 256;
    size_t length = 0;
    ...
    if (!buffer) {
        error_alert(l, "out of memory");
    }

    /*
     * Current position is the first "
     * of """
     */
    advance(l);
    advance(l);
    ...
    if (l->current == '"') {
        ...
        */
        if (
            ...
            t.type = VTOKEN_STRING;
            t.value = read_multiline_string(l);
            return t;
        }
...
Value eval(ASTNode* node) {
    ...
    return ((Value){0});
        ...
        case AST_CALL: {
            ...
            /* file_exists(path) */
            if (strcmp(node->call.name, "file_exists") == 0) {

                if (node->call.arg_count != 1) {
                    runtime_error("file_exists() expects 1 argument");
                    return ((Value){0});
                }

                Value path = eval(node->call.args[0]);

                if (path.type != 1) {
                    runtime_error("file_exists() expects a string");
                    return ((Value){0});
                }

                Value v = {0};
                v.type = 2;
                v.num = file_exists(path.s);
            ...
            /* file_read(path) */
            if (strcmp(node->call.name, "file_read") == 0) {

                if (node->call.arg_count != 1) {
                    runtime_error("file_read() expects 1 argument");
                    return ((Value){0});
                }
                ...
                if (path.type != 1) {
                    runtime_error("file_read() expects a string");
                    return ((Value){0});
                }
                ...
                if (size < 0) {
                    fclose(f);
                    runtime_error("cannot determine file size");
                    return ((Value){0});
                }
            ...
            if (strcmp(node->call.name, "file_write") == 0) {

                if (node->call.arg_count != 2) {
                    runtime_error("file_write() expects 2 arguments");
                    return ((Value){0});
                }

                Value path = eval(node->call.args[0]);
                Value data = eval(node->call.args[1]);

                if (path.type != 1 || data.type != 1) {
                    runtime_error(
                        "file_write() expects two strings"
                    );
                    return ((Value){0});
                }
                ...
                if (!f) {
                    runtime_error("cannot open file for writing");
                    return ((Value){0});
                }
            ...
            if (strcmp(node->call.name, "file_append") == 0) {

                if (node->call.arg_count != 2) {
                    runtime_error("file_append() expects 2 arguments");
                    return ((Value){0});
                }
                ...
                if (path.type != 1 || data.type != 1) {
                    runtime_error(
                        "file_append() expects two strings"
                    );
                    return ((Value){0});
                }
                ...
                if (!f) {
                    runtime_error("cannot open file for appending");
                    return ((Value){0});
                }
            ...
            /* file_remove(path) */
            if (strcmp(node->call.name, "file_remove") == 0) {

                if (node->call.arg_count != 1) {
                    runtime_error("file_remove() expects 1 argument");
                    return ((Value){0});
                }
                ...
                if (path.type != 1) {
                    runtime_error("file_remove() expects a string");
                    return ((Value){0});
                }

                if (remove(path.s) != 0) {
                    runtime_error("cannot remove file");
                    return ((Value){0});
                }
