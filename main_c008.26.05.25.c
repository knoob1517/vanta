...
typedef struct { int type; double i; char* s; } Value;
int break_flag = 0;
int continue_flag = 0;
int return_flag = 0;
Value return_value;
...
Value eval(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_RAND: {
            ...
            else v.i = ceil((rand() % ((int)max_val.i - (int)min_val.i + 1))) + floor(min_val.i);
            return v;
        }
        case AST_BUILTIN: {
            ...
            switch(node->builtin.type) {
                ...
                case VTOKEN_ABS: v.type = 0; v.i = (arg.type == 0) ? abs((int)arg.i) : 0; break;
                case VTOKEN_FABS: v.type = 0; v.i = (arg.type == 0) ? fabs(arg.i) : 0; break;
                case VTOKEN_SQRT: {
                    if (arg.type != 0) {
                        printf("runtime error: sqrt of non-number\n");
                        exit(1);
                    }

                    if (arg.i < 0) {
                        printf("runtime error: sqrt of negative number: %g\n", arg.i);
                        exit(1);
                    }
        ...
        case AST_BINOP: {
            ...
            else if (node->binop.op == VTOKEN_MOD)   v.i = (r.i != 0) ? fmod(l.i, r.i) : 0;
            else if (node->binop.op == VTOKEN_EQ)    {v.type = 2; v.i = fabs(l.i - r.i) < 1e-9;}
            else if (node->binop.op == VTOKEN_NEQ)   {v.type = 2; v.i = fabs(l.i - r.i) >= 1e-9;}
            else if (node->binop.op == VTOKEN_G)     {v.type = 2; v.i = (l.i >  r.i);}
            else if (node->binop.op == VTOKEN_S)     {v.type = 2; v.i = (l.i <  r.i);}
            else if (node->binop.op == VTOKEN_GEQ)   {v.type = 2; v.i = (l.i >= r.i);}
            else if (node->binop.op == VTOKEN_SEQ)   {v.type = 2; v.i = (l.i <= r.i);}
            else if (node->binop.op == VTOKEN_AND)   {v.type = 2; v.i = (l.i && r.i);}
            else if (node->binop.op == VTOKEN_OR)    {v.type = 2; v.i = (l.i || r.i);}
            return v;
        }
        case AST_IF: {
