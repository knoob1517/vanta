...
typedef enum {
    ...
    AST_STRING, AST_BUILTIN, AST_RAND, AST_IF,
} ASTType;
...
ASTNode* parse_expo(Parser* p) {
    ...
    if (p->current.type == VTOKEN_EXPO) {
        ...
        ASTNode* right = parse_expo(p); // 🔥 recursion
...
Value eval(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_BINOP: {
            ...
            else if (node->binop.op == VTOKEN_MOD) v.i = l.i % r.i;
            return v;
        }
        case AST_IF: {
...
char* read_file(const char* filename) {
    ...
    if (!f) {
        // 🔥 Thông báo lỗi nếu không tìm thấy file cụ thể
...
int main(int argc, char* argv[]) {
    ...
    // 🔥 Logic kiểm tra đối số dòng lệnh
    if (argc > 2) {
        printf("usage: vanta [filename]\n");
        return 1;
    } 
    else if (argc == 2) {
        // Chạy file người dùng chỉ định: .\vanta script.vt
        filename = argv[1];
    } 
    else {
        // Mặc định chạy test.vt nếu không nhập gì: .\vanta
