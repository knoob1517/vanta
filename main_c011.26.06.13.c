...
typedef enum {
    ...
    AST_LIST, AST_INDEX, AST_SPLIT, AST_IASSIGN,
} ASTType;
...
typedef struct { char* name; Value value; int is_const; } Variable;
...
void set_var(const char* name, Value val, int is_const) {
    for (int i = 0; i < var_count; i++) {
        ...
        if (strcmp(vars[i].name, name) == 0) {
            ...
            if (vars[i].is_const) {
                printf(
                    "runtime error: cannot modify const '%s'\n",
                    name
                );
                exit(1);
            }
...
Value eval(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_LET: { Value val = eval(node->let.value); set_var(node->let.name, val, 0); return val; }
        case AST_CONST: {
            Value val =
                eval(node->let.value);
        ...
        case AST_IASSIGN: {
            if (!is_variable_defined(node->let.name)) {
                printf("runtime error: undefined variable '%s'\n", node->let.name);
                exit(1);
            } 
            Value val = eval(node->let.value); 
            set_var(node->let.name, val, 0); 
            return val; 
        }
        case AST_PRINT: {
            ...
            for (int i = 0; i < func_count; i++) {
                if (strcmp(functions[i].name, node->call.name) == 0) {
                    ...
                    for (int j = 0; j < fn->function.param_count; j++) {
                        ...
                        set_var(fn->function.params[j], arg, 0);
                    }
